@echo off
echo ========================================
echo התקנת תוסף COM עם קובץ CMD
echo ========================================
...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "LoadBehavior" /t REG_DWORD /d 3 /f
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "FriendlyName" /t REG_SZ /d "CMD Addin" /f
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "Description" /t REG_SZ /d "CMD Addin for Testing" /f

echo.
echo 4. מוסיף הרשאות אימות...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "Authentication" /t REG_DWORD /d 1 /f

echo.
echo 5. מוסיף נתיב לקובץ...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "Path" /t REG_SZ /d "%CD%\cmd_addin.py" /f

echo.
echo 6. מוסיף נתיב ל-Python...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "PythonPath" /t REG_SZ /d "python" /f

echo.
echo 7. מוסיף נתיב ל-CMD...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin" /v "CMDPath" /t REG_SZ /d "cmd.exe" /f

echo.
echo 8. בודק את הרישום...
reg query "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\CMDAddin.Addin"
