...
echo.
echo שלב 3: רישום התוסף ב-COM...
echo.

:: רישום התוסף ב-COM
python outlook_com_addin_registered.py --register
if %errorLevel% neq 0 (
...
echo.
echo שלב 4: רישום התוסף ב-Outlook...
echo.
...
echo.
echo שלב 5: יצירת קיצורי דרך...
echo.

:: קיצור דרך על שולחן העבודה
echo [InternetShortcut] > "%USERPROFILE%\Desktop\AI Email Manager.url"
echo URL=file:///%CD%/outlook_com_addin_registered.py >> "%USERPROFILE%\Desktop\AI Email Manager.url"
...
echo URL=file:///%CD%/outlook_com_addin_registered.py >> "%APPDATA%\Microsoft\Windows\Start Menu\Programs\AI Email Manager.url"
...
echo    python outlook_com_addin_registered.py
...
echo.
echo 📞 תמיכה:
echo - בדוק את הלוגים ב-outlook_com_addin_registered.log
