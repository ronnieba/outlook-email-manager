...
python ps1_addin2.py

echo.
echo 3. מעדכן את רישום Outlook...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "LoadBehavior" /t REG_DWORD /d 3 /f
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "FriendlyName" /t REG_SZ /d "PS1 Addin 2" /f
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "Description" /t REG_SZ /d "PS1 Addin 2 for Testing" /f

echo.
echo 4. מוסיף הרשאות אימות...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "Authentication" /t REG_DWORD /d 1 /f

echo.
echo 5. מוסיף נתיב לקובץ...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "Path" /t REG_SZ /d "%CD%\ps1_addin2.py" /f

echo.
echo 6. מוסיף נתיב ל-Python...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "PythonPath" /t REG_SZ /d "python" /f

echo.
echo 7. מוסיף נתיב ל-PS1...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin" /v "PS1Path" /t REG_SZ /d "powershell.exe" /f

echo.
echo 8. בודק את הרישום...
reg query "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\PS1Addin2.Addin"
