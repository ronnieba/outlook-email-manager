@echo off
echo ========================================
echo התקנת תוסף COM עם קובץ BAT
echo ========================================
...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "LoadBehavior" /t REG_DWORD /d 3 /f
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "FriendlyName" /t REG_SZ /d "BAT Addin" /f
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "Description" /t REG_SZ /d "BAT Addin for Testing" /f

echo.
echo 4. מוסיף הרשאות אימות...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "Authentication" /t REG_DWORD /d 1 /f

echo.
echo 5. מוסיף נתיב לקובץ...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "Path" /t REG_SZ /d "%CD%\bat_addin.py" /f

echo.
echo 6. מוסיף נתיב ל-Python...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "PythonPath" /t REG_SZ /d "python" /f

echo.
echo 7. מוסיף נתיב ל-BAT...
reg add "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin" /v "BATPath" /t REG_SZ /d "cmd.exe" /f

echo.
echo 8. בודק את הרישום...
reg query "HKEY_CURRENT_USER\Software\Microsoft\Office\Outlook\Addins\BATAddin.Addin"
